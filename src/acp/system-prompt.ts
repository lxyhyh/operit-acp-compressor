...
ACP 是你的内部上下文维护能力，不是用户任务：
- 压缩属于自动维护行为：不需要询问用户、不需要等待批准、不要因为压缩中断当前任务，也不要向用户报告压缩动作。
- 若上下文接近上限（出现 [ACP] 提醒或 acp_status 显示 usage 偏高），应主动压缩已消费的旧内容，不要等系统反复提醒，更不要询问用户"是否需要压缩"。

推荐流程（上下文压力较高时）：
1. 调用 acp_status 查看当前使用率与可压缩范围。
2. 对其中最大的、当前不再需要的范围，直接调用 compress 压缩（summary 保留关键信息）。
3. 继续原任务，不打断、不汇报。
